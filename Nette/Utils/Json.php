...
class Json
{
	...
	public static function encode($value, $options = 0)
	{
		...
		$json = json_encode(
			$value,
			PHP_VERSION_ID >= 50400 && !defined('HHVM_VERSION') ? (JSON_UNESCAPED_UNICODE | ($options & self::PRETTY ? JSON_PRETTY_PRINT : 0)) : 0
		);
		restore_error_handler();
		if (isset($old)) {
	...
	public static function decode($json, $options = 0)
	{
		...
		$args[] = 512;
		if (PHP_VERSION_ID >= 50400 && !(defined('JSON_C_VERSION') && PHP_INT_SIZE > 4) && !defined('HHVM_VERSION')) { // not implemented in PECL JSON-C 1.3.2 for 64bit systems
